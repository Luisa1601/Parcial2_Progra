...
public class ComisionesImpl implements IComisiones{
    ...
    @Override
    public String iniciarSesion(mdLogin login) {
        mdLogin loginDb = new mdLogin();
        String usuario = "";
        
        try {
           loginDb = datos.obtenerUsuario(login);
            
             usuario = loginDb.getUsuario();
            
        } catch (SQLException ex) {
            Logger.getLogger(ComisionesImpl.class.getName()).log(Level.SEVERE, null, ex);
        }
